/* 
 * Original ArticleViewPage - commented out for reference
 * This file contains the original implementation with all interactive features
 * We'll restore these features after fixing the basic version
 */
...
// Mock article data for fallback when real data cannot be loaded
const MOCK_ARTICLE = {
  id: '1',
  title: '인공지능의 발전과 미래',
...
// Mock translation data with type safety
...
const ArticleViewPage = () => {
  ...
  useEffect(() => {
    const fetchArticle = async () => {
      ...
      try {
        // Fetch real article data from the API
        console.log(`Fetching article with ID: ${articleId}`);
        const response = await api.get(`/api/articles/${articleId}`);
        
        // If we got a successful response with article data
        if (response.data && response.status === 200) {
          console.log('Article data received:', response.data);
          setArticle(response.data);
          
          // Try to fetch quiz data for this article
          try {
            const quizResponse = await api.get(`/api/quizzes/${articleId}`);
            if (quizResponse.data && quizResponse.status === 200) {
              setQuiz(quizResponse.data);
            } else {
              // Fallback to mock quiz if the API doesn't return quiz data
              console.log('Using mock quiz data');
              setQuiz(MOCK_QUIZ);
            }
          } catch (quizErr) {
            console.warn('Error fetching quiz, using mock data:', quizErr);
            setQuiz(MOCK_QUIZ);
          }
        } else {
          // If we didn't get article data, fall back to mock data
          console.warn('No article data received from API, using mock data');
        ...
        console.log('API call failed, using mock article data');
        setArticle(MOCK_ARTICLE);
        setQuiz(MOCK_QUIZ);
        
        // Show a more descriptive error message in the toast
        let errorMessage = 'Could not load article. Using sample article instead.';
        
        if (err.response && err.response.status === 404) {
          errorMessage = `Article not found (ID: ${articleId}). Using sample article instead.`;
          setError(`Article not found. Article ID: ${articleId}`);
        } else {
          setError('Failed to load the article. Using sample content instead.');
        }
        
        toast({
          title: "Couldn't load real article",
    ...
    if (articleId) {
      ...
      // If no article ID is provided, use mock data
      setArticle(MOCK_ARTICLE);
      setQuiz(MOCK_QUIZ);
      setIsLoading(false);
    }
  ...
  const translateText = async (text: string) => {
    ...
    try {
      // Try to fetch real translation from the API
      try {
        ...
        // If we got a successful response with translation data
        if (response.data && response.status === 200) {
          ...
          try {
            await axios.post('http://localhost:8000/api/vocabulary/add', {
              word: text,
              language: article?.language || 'ko',
              translation: response.data.translation,
              source_article: articleId
            });
            console.log(`Added to vocabulary: ${text}`);
          } catch (vocabErr) {
            console.warn('Error adding to vocabulary:', vocabErr);
          }
          return;
        }
      } catch (apiErr) {
        console.warn('Error fetching translation from API, using fallback:', apiErr);
      }
